...
let c1 = createCanvas({ width: $(window).width(), height: $(window).height()/7 });
let c2 = createCanvas({ width: $(window).width(), height: $(window).height()/7 });
let c3 = createCanvas({ width: $(window).width(), height: $(window).height()/7 });

let tela = c1.canvas;
let canvas = c1.context;

// $("body").append(tela);
$("#text").append(c3.canvas);
writeText(c2.canvas, c2.context, "LA VOITURE AUTONOME");
...
class Particle {
  constructor(canvas, options) {
    ...
    this.color = this.radius > 5 ? "#c4e0e5" : "#4ca1af"; //this.randomColor()
  }

  randomColor() {
    let colors = ["#4ca1af", "#FFFFFF"];
